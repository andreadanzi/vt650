...
class MailManager_Relate_Action extends Vtiger_MailScannerAction {
	...
	public static function associate($mailrecord, $linkto) {
	    global $log;
	    $log->debug("MailManager_Relate_Action::associate to ".$linkto);
		$instance = new self();

		$modulename = getSalesEntityType($linkto);
	    $log->debug("MailManager_Relate_Action::associate module ".$modulename);
		...
		if (!empty($emailid)) {
	        $log->debug("MailManager_Relate_Action::associate emailid = ".$emailid. " to crimid = " . $linkfocus->id);
	    ...
	    $log->debug("MailManager_Relate_Action::associate terminated ". $name[$linkto]);
