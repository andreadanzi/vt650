...
class MailManager {
	...
	static function lookupMailAssociation($mailuid) {
		global $adb, $log;
        $log->debug("MailManage::lookupMailAssociation mailuid=".$mailuid);
		// Mail could get associated with two-or-more records if they get deleted after linking.
		$sql = "SELECT vtiger_mailmanager_mailrel.* FROM vtiger_mailmanager_mailrel INNER JOIN
        ...
        $log->debug("MailManage::lookupMailAssociation sql=".$sql);
		$result = $adb->pquery($sql , array(html_entity_decode($mailuid)));
		if ($adb->num_rows($result)) {
			$resultrow = $adb->fetch_array($result);
			return $resultrow;
		}
		$log->debug("MailManage::lookupMailAssociation return false" );
		return false;
	}
	...
	static function lookupVTEMailAssociation($emailId) {
		...
		$result = $adb->pquery(
			...
			vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_mailmanager_mailrel.crmid AND vtiger_crmentity.deleted=0
			AND vtiger_mailmanager_mailrel.mailuid=? LIMIT 1", array(html_entity_decode($mailuid)));
		if ($adb->num_rows($result)) {
			$resultrow = $adb->fetch_array($result);
			return $resultrow;
		}
