	...
	MailManager = {
		...
		*/
		...
		mail_associate_create_wizard: function(form){
			...
			var frmparams = Form.serialize(form);
			// danzi.tn#20 - 20170829 - gestione progetti non associati
			if(form._mlinktotype.value == 'Project') {
			    MailManager.showSelectProject(form,frmparams);	
			    progressIndicatorElement.progressIndicator({'mode' : 'hide'});
			    // qualcosa simile a MailManager.showCommentWidget(form);	
			    // vedi Detail.js del modulo VTiger button.selectRelation
			} else {
		...
		mail_associate_actions : function(form) {
            ...
            // danzi.tn#20 - 20170907 - gestione progetti
			// No record is selected for linking?
			if (selected == false && form._mlinktotype.value != 'Project' ) {
        ...
        showSelectProject: function(form,frmparams) {
                ...
                var thisInstance = this;
			    var params = { module: "Project",
			                   eventName: "selectProjectMailManager",
			                   src_field: "project_id",
                                src_module: "MailManager",
                                src_record:_msgno };
			    ...
			    popupInstance.show(params,function(data){
				    ...
				    var editor = selectedItem;
				    jQuery('#mailproject_id').val( selectedItem["id"]);			
				    frmparams = frmparams+'&mailproject_name='+selectedItem["name"]+'&mailproject_id='+selectedItem["id"];
				    AppConnector.request(MailManager._baseurl() + "_operation=relation&_operationarg=link&" + frmparams).then(function(responseJSON) {
                        var responseObject = JSON.parse(responseJSON);
                        var resultJSON = responseObject.result; //responseObject.result.linkto , responseObject.result.linktotype
                        var responseUrl = "index.php?module="+ responseObject.result.linktotype + "&view=Detail&record=" + responseObject.result.linkto;
