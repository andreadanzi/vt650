...
{strip}
	{foreach item=HEADER from=$RELATED_HEADERS}
		...
		{elseif $HEADER->get('label') eq "Status"}
			{assign var=TASK_STATUS_HEADER value={vtranslate($HEADER->get('label'),$MODULE)}}
		{/if}
	{/foreach}
	<div class="row-fluid">		
		<span class="span7">
		...
		<span class="span4">
			<span class="pull-right">
				<strong>{$TASK_STATUS_HEADER}</strong>
			</span>
		</span>
	</div>
				...
				<li>
					<div class="row-fluid">
						...
						<span class="span4 horizontalLeftSpacingForSummaryWidgetContents">
						    <!-- danzi.tn#15 - projecttaskprogress changed to projecttaskstatus -->
							<span class="pull-right">{$RELATED_RECORD->getDisplayValue('projecttaskstatus')}</span>
						</span>
					</div>
				</li>
			</ul>
