					...
					<div class="widget_header row-fluid">
						...
						<span class="span3">
							{if $COMMENTS_WIDGET_MODEL->get('action')}
								<button class="btn btn-success pull-right addButton createRecord" type="button" data-url="{$COMMENTS_WIDGET_MODEL->get('actionURL')}">
					...
					<div class="widget_header row-fluid">
						...
						<span class="span3">
							{if $HELPDESK_WIDGET_MODEL->get('action')}
								<button class="btn btn-success pull-right addButton createRecord" type="button" data-url="{$HELPDESK_WIDGET_MODEL->get('actionURL')}">
									<i class="fa fa-plus" title="{vtranslate('LBL_ADD',$MODULE_NAME)}"></i>
								</button>
							{/if}
						</span>
						...
						<span class="span3">
							<span class=" pull-right">
								{if $MILESTONE_WIDGET_MODEL->get('action')}
									<button class="btn btn-success addButton" id="createProjectMileStone" type="button" data-url="{$MILESTONE_WIDGET_MODEL->get('actionURL')}" data-parent-related-field="projectid">
										<i class="fa fa-plus" title="{vtranslate('LBL_ADD',$MODULE_NAME)}"></i>
									</button>
								{/if}
							</span>
					...
					<div class="widget_header row-fluid">
						...
						<span class="span3">
							{if $TASKS_WIDGET_MODEL->get('action')}
								<button class="btn btn-success pull-right addButton" id="createProjectTask" type="button" data-url="{$TASKS_WIDGET_MODEL->get('actionURL')}" data-parent-related-field="projectid">
									<i class="fa fa-plus" title="{vtranslate('LBL_ADD',$MODULE_NAME)}"></i>
								</button>
							{/if}
						</span>
						...
						<span class="span9">
							<div class="row-fluid">
								...
								<span class="span7">
									...
									{assign var="SPECIAL_VALIDATOR" value=$FIELD_MODEL->getValidator()}
									<select style="width: 160px;" class="chzn-select" name="{$FIELD_MODEL->get('name')}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-fieldinfo='{$FIELD_INFO|escape}' {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} >
										<option>{vtranslate('LBL_SELECT_PROGRESS',$MODULE_NAME)}</option>
										{foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$PICKLIST_VALUES}
											<option value="{$PICKLIST_NAME}" {if $FIELD_MODEL->get('fieldvalue') eq $PICKLIST_NAME} selected {/if}>{$PICKLIST_VALUE}</option>
										{/foreach}
									</select>
					...
					<div class="widget_header row-fluid">
						...
						<span class="span3">
							{if $DOCUMENT_WIDGET_MODEL->get('action')}
								<button class="btn btn-success pull-right addButton createRecord" type="button" data-url="{$DOCUMENT_WIDGET_MODEL->get('actionURL')}">
									<i class="fa fa-plus" title="{vtranslate('LBL_ADD',$MODULE_NAME)}"></i>
								</button>
							{/if}
						</span>
					...
					<div class="widget_header row-fluid">
						...
						<span class="span3">
							{if $UPDATES_WIDGET_MODEL->get('action')}
								<button class="btn btn-success pull-right addButton createRecord" type="button" data-url="{$UPDATES_WIDGET_MODEL->get('actionURL')}">
									<i class="fa fa-plus" title="{vtranslate('LBL_ADD',$MODULE_NAME)}"></i>
								</button>
							{/if}
						</span>
